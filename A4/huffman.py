...
class Huffman:
    ...
    def codeBuilder(self):
        ...
        while len(heap) > 1:
            ...
            for pair in two[1:]:
                pair[1] = '1' + pair[1]
            heappush(heap, [one[0] + two[0]] + one[1:] + two[1:])
...
# create dictionary containing the array of canonical collection
dictFiles = {"num1": dictFiles1,
         "num2": dictFiles2,
         "num3": dictFiles3}

for x,y in dictFiles.items():
    inText = ""
    ...
    for i, fname in enumerate(inFiles):
       oSum += os.path.getsize(fname)
       eSum += fLen[i]
    
    # to ensure file gets rewritten and not added to each time the program is run
    open("results" + str(x) + ".txt", "w").close()
    # calculate average
    oAvg = oSum/5
    eAvg = eSum/5
    # the name of this file is updated for each canonical collection, results1 = Collection1, results2 = Collection2...
    outF = open("results" + str(x) + ".txt", "a")
    outF.write("Average Original size of: " + str(oAvg) + "   Average Encoded size: " + str(eAvg)+'\n')
    if oAvg < eAvg:
        outF.write("Since the average original size is smaller than the averag encoded size, it is not worth encoding\n")
    else:
        outF.write("Since the average original size is larger than the average encoded size, it is worth encoding\n")
    outF.close()
