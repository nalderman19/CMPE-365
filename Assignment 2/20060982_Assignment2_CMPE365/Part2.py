...
#loops through example set sizes
for sizes in range(4,15):
	BFIAvg = 0
	HSAAvg = 0
	#perform this many tests
	for i in range(1,20):
		randS = []
		targets = []
		#fill up random sets and random targets
		for i in range(1,sizes):
			randS.append(random.randint(1,750))
		for k in range(0,10):
			targets.append(random.randint(1,750))
		BFISum = 0
		HSASum = 0
		#Use BFI algorithm and HS algorithm to get sums
		for target in targets:
			BFISum += BFI_SS(randS, target)[1]
			HSASum += HS_SS(randS, target)[1]
		#calculate averages
		BFIAvg += BFISum / len(targets)
		HSAAvg += HSASum / len(targets)
	#calculate scores
	BFIScore.append(int(BFIAvg / sizes))
	HSAScore.append(int(HSAAvg / sizes))

#format table for scores for each set size
print("Size \t\t BFI Alg  \t HS Alg")
for i in range(0,11):
	if BFIScore[i] < 1000:
		print(str(i + 4) + "\t\t" + str(BFIScore[i]) + "\t" + str(HSAScore[i]))
	if BFIScore[i] > 1000:
		print(str(i + 4) + "\t\t" + str(BFIScore[i]) + "\t" + str(HSAScore[i]))
